...
### 기본 흐름
1. 최근 및 전체 당첨 번호 데이터를 로드해 번호별 빈도수를 계산합니다.
2. 전체 빈도 40% + 최근 30회 빈도 60%로 번호별 가중치를 계산합니다.
3. 최근 15회에 나오지 않은 번호는 콜드 패널티로 가중치를 50% 낮춥니다.
4. 가중치 기반 랜덤 추출로 중복 없는 6개 번호를 생성합니다.
5. 아래 공통 필터를 먼저 검사합니다.
   - 합계 `100 ~ 175`
   - 홀수 개수 `2 ~ 4`
   - `3연번 이상 금지`
6. 공통 필터 통과 후 세트별 특화 조건을 적용합니다.
7. 조건 충족 시 결과를 반환하고, 실패하면 최대 200회까지 재시도합니다.
